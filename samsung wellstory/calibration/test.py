...
try:
    while True:
        ...
        if key == ord('q'):
            ...
            running = True
            last_visited_rob_xyz = None  # 직전에 방문한 접시 위치
            cycle = 0
            ...
            while running:
                ...
                if k == ord('x'):
                    print(f"  [중지] x키로 루프 종료")
                    break

                # Start Point에서 실시간 접시 인식
                for _ in range(3):  # 안정된 프레임 확보
                    frames = pipeline.wait_for_frames()
                aligned = align.process(frames)
                ...
                filt = spatial.process(df)
                filt = temporal.process(filt)
                df_filt = filt.as_depth_frame()
                ...
                for r in results:
                    if r.masks is not None:
                        for i, box in enumerate(r.boxes):
                            conf = float(box.conf[0])
                            mask = r.masks.data[i].cpu().numpy()
                            center_3d, ecx, ecy = detect_bowl_center(mask, df_filt, intr)
                            if center_3d is not None:
                                cam_xyz = center_3d.tolist()
                                rob_xyz = cam_to_robot(cam_xyz)
                                bowls_found.append((cam_xyz, rob_xyz, ecx, ecy, conf))
                ...
                if not bowls_found:
                    print(f"\n  [Cycle {cycle}] 접시 없음 → 대기 중... (접시를 놓으면 재개)")
                    # 접시가 생길 때까지 계속 detect
                    while running:
                        ...
                        if not cf2 or not df2:
                            continue
                        filt2 = spatial.process(df2)
                        filt2 = temporal.process(filt2)
                        df_filt2 = filt2.as_depth_frame()
                        img2 = np.asanyarray(cf2.get_data())
                        intr2 = df2.profile.as_video_stream_profile().intrinsics
                        cv2.putText(img2, "WAITING FOR BOWLS...", (300, 360),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
                        cv2.imshow('Multi-Bowl Test', img2)
                        # 재인식 시도
                        res2 = model(img2, conf=0.7, verbose=False)
                        bowls_found = []
                        for r2 in res2:
                            if r2.masks is not None:
                                for i2, box2 in enumerate(r2.boxes):
                                    mask2 = r2.masks.data[i2].cpu().numpy()
                                    c3d, ex, ey = detect_bowl_center(mask2, df_filt2, intr2)
                                    if c3d is not None:
                                        cxyz = c3d.tolist()
                                        rxyz = cam_to_robot(cxyz)
                                        bowls_found.append((cxyz, rxyz, ex, ey, float(box2.conf[0])))
                        if bowls_found:
                            print(f"  [재개] 접시 {len(bowls_found)}개 감지 → 루프 재개")
                            last_visited_rob_xyz = None  # 대기 후 재시작이므로 초기화
                            break
                    if not running:
                        break
                    if not bowls_found:
                ...
                # 거리 순 정렬
                bowls_found.sort(key=lambda b: dist_from_start(b[1]))
                print(f"\n  [Cycle {cycle}] {len(bowls_found)}개 인식")

                # 직전 방문 접시 제외하고 선택
                selected = None
                if last_visited_rob_xyz is not None and len(bowls_found) > 1:
                    # 직전 방문과 가장 가까운 bowl 찾기 → 제외
                    def dist_to_last(b):
                        dx = b[1][0] - last_visited_rob_xyz[0]
                        dy = b[1][1] - last_visited_rob_xyz[1]
                        return math.sqrt(dx*dx + dy*dy)
                    last_idx = min(range(len(bowls_found)), key=lambda i: dist_to_last(bowls_found[i]))
                    for i, b in enumerate(bowls_found):
                        if i != last_idx:
                            selected = b
                            break

                if selected is None:
                    selected = bowls_found[0]

                cam_xyz, rob_xyz, ecx, ecy, conf = selected
                last_visited_rob_xyz = rob_xyz
                ...
                print(f"  [→] ({target_x*1000:.0f}, {target_y*1000:.0f}, {target_z*1000:.0f})mm 이동")
                ...
                if result and '1' in str(result):
                    print(f"  [!] Bowl 이동 실패 (작업 범위 초과) → Start 복귀 후 다음 사이클")
                    robot.Movej_Cmd(START_JOINTS, v=V_MOVE, r=0)
                    last_visited_rob_xyz = None
                    continue
